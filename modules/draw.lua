...
function draw.DrawText( text, font, x, y, colour, xalign )
	...
	surface.SetFont( font )
	local _, lineHeight = GetTextSize( "\n" )
	local tabWidth = 50
		...
		if #str > 0 then
			if find( str, "\t" ) then -- there's tabs, some more calculations required
				for tabs, str2 in gmatch( str, "(\t*)([^\t]*)" ) do
					...
					if #str2 > 0 then
						draw.SimpleText( str2, font, curX, curY, colour, xalign )
				...
				draw.SimpleText( str, font, curX, curY, colour, xalign )
			end
		else
			curX = x
...
function draw.Text( tab )
	return draw.SimpleText( tab.text, tab.font, tab.pos[ 1 ], tab.pos[ 2 ], tab.color, tab.xalign, tab.yalign )
end
...
function draw.Circle( x, y, radius, seg, color )
	...
	local a = rad( 0 ) -- This is needed for non absolute segment counts
	table_insert( cir, { x = x + sin( a ) * radius, y = y + cos( a ) * radius, u = sin( a ) / 2 + 0.5, v = cos( a ) / 2 + 0.5 } )
...
function draw.Gradient(x, y, w, h, dir, color)

	surface.SetDrawColor( color )

	surface.SetMaterial( dir == 1 and Gradients.GradientLeft or dir == 2 and Gradients.GradientRight or dir == 3 and Gradients.GradientUp or Gradients.GradientDown )
	surface.DrawTexturedRect( x, y, w, h )
end


function draw.Line(s_x, s_y, e_x, e_y, color)
	surface.SetDrawColor( color )
	surface.DrawLine( s_x, s_y, e_x, e_y )
end

function draw.Hitmarker()
	hitmarker.lasthit = SysTime()
